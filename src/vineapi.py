...
class Vine(object):
    ...
    def login(self, username, password):
        response = self._call("users/authenticate", \
                              data={"username": username, "password": password})
        self._user_id = response["data"]["userId"]
        self._key = response["data"]["key"]

    def timeline_sections(self, section, page=None, size=None):
        return self._call("/timelines/channels/%s/popular" % section, \
                                    params={"page": page, "size": size})["data"]
